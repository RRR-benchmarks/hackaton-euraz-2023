...
export namespace DropDownList {
  ...
  export function DropDownList(props: DropDownListProps) {
    ...
    useEffect(() => {
      if (hoveredItem) setHoveredItem(props.items.find(it => it.type === hoveredItem.type && it.id === hoveredItem.id));
    }, [props.items]);
    useEffect(() => {
      if (hoveredItem) props.setSelectedBearers([hoveredItem]);else props.setSelectedBearers([]);
    }, [hoveredItem]);
    const setHovered = (item: ItemType, isHovered: boolean, ...message: string[]) => {
      //console.log('setHovered', item, isHovered, message[0])
      if (isHovered) setHoveredItem(item);else setHoveredItem(undefined);
    };
    return <View
